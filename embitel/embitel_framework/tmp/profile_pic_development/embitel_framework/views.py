...
from embitel_framework.models import P12Certificate, APNSDevices, GCMDevices
import commands
...
@csrf_exempt
def set_profile_pic(request, device=None):
    try:
        device = APNSDevices.objects.get(phone_number='1234567756')
    except:
        pass
    try:
        data = eval(str(request.body))
...
def register_urbanairship_devices(request, *args, **kwargs):
    ...
    try:
        ...
        device.save()
        if request.FILES:
            result = set_profile_pic(request, device)
...
def verify_user(request, *args, **kwargs):
    ...
    try:
        ...
        result = [{'success': True,  'phone_number': str(phone_number)}]
    except:
        result = [{'success': False,  'phone_number': ''}]
...
def verify_registered_users(request, *args, **kwargs):
    ...
    try:
        import ast
        registered_ios_devices = APNSDevices.objects.filter(app_key=app_key, phone_number__in=phone_number).values_list('phone_number', flat='true')
        registered_android_devices = GCMDevices.objects.filter(app_key=app_key, phone_number__in=phone_number).values_list('phone_number', flat='true')
...
def send_PN_to_device(request, *args, **kwargs):
    ...
    model = get_model(phone_number=phone_number)        
    print model
    devices = list(model.objects.filter(phone_number=phone_number, app_key=app_key, pn_status=True))
    print devices
    for device in devices:
        ...
        if data.has_key('delivered') and device.device_type=='ios':
            COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":"%s"},"device_tokens": %s,"phone_number":"%s", "from_phone_number":"%s", "delivered":"%s", "message_id":"%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, message, [str(device.registration_id)], device.phone_number, from_phone_number, data['message'], message_id)
        elif device.device_type == 'ios':
            COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":"%s"},"device_tokens": %s,"phone_number":"%s", "from_phone_number":"%s", "message_id": "%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, message, [str(device.registration_id)], device.phone_number, from_phone_number, message_id)

        elif device.device_type=='android':
            print 3
            #COMMAND = """curl -X POST -u "%s:%s"   -H "Content-type: application/json" -H "Accept: application/vnd.urbanairship+json; version=3;" --data '''{"audience":{"apid" : "%s"}, "notification":{"alert" : "%s"},"device_types":["android"], "message_id":"%s", "from_phone_number":"%s", "phone_number":"%s"}''' https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, device.registration_id, message, message_id, from_phone_number, phone_number)
            COMMAND = """curl -X POST -u "%s:%s"   -H "Content-type: application/json" -H "Accept: application/vnd.urbanairship+json; version=3;" --data '''{"audience":{"apid" : "%s"}, "notification":{"alert" : "%s"},"device_types":["android"]}''' https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, device.registration_id, message)
