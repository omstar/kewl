...
def register_urbanairship_devices_get(request, *args, **kwargs):
    try:
        ...
        result = [{'success': True, 'phone_number':phone_number, 'profile_pic':device.profile_pic}]
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", result
    except Exception, e:
        print str(e)
        result = [{'success': False,  'phone_number': ''}]
...
@csrf_exempt
def authenticate_user(request, *args, **kwargs):
    try:
        data = eval(str(request.body))
        phone_number = data['phone_number']
        otp = data['otp']
    ...
    for device in devices:
        ...
        if pending_messages:
            ...
            if device.device_type == 'ios':
                print 3 
                try:
                    COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":"%s"},"device_tokens": %s,"phone_number":"%s", "from_phone_number":"%s", "message_id": "%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key,pending_message.message, [str(device.registration_id)], device.phone_number, pending_message.from_phone_number, pending_message.message_id)
                    COMMAND = COMMAND.replace("\'","\"" ).replace("\"\"\"", '\'\'\'')
                ...
                print 4
                try:
                    COMMAND = """curl -X POST -u "%s:%s"   -H "Content-type: application/json" -H "Accept: application/vnd.urbanairship+json; version=3;" --data '''{"audience":{"apid" : "%s"}, "notification":{"alert" : "%s", "android":{"extra":%s}},"device_types":["android"]}''' https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, device.registration_id, message, {"phone_number":device.phone_number, "from_phone_number":pending_message.from_phone_number, "message_id": pending_message.message_id})
                    print 5
                    COMMAND = COMMAND.replace("\'","\"" ).replace("\"\"\"", '\'\'\'')
                    print 6
                    commands.getoutput(COMMAND)
