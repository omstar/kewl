...
from embitel_framework.models import P12Certificate, APNSDevices, GCMDevices, Messages, Groups, Channels,  log_visit
import commands

from django.contrib.auth import authenticate, login
from django.contrib.auth.models import User, AnonymousUser

from forms import LoginForm, UploadP12Certificate, RegistrationForm, ConfigureNotificationsForm
...
def login_proc(request):
    ...
    log_visit(request=request, action = 'LOGIN_TRY', email= email, source="WEB")
    ...
    if request.user.is_staff:
        logger.info("%s (Developer) Logged in successfully!" %email)
        ...
        log_visit(request=request, action = 'LOGGED_IN', email= email, source="WEB")
        response =  HttpResponseRedirect('/embitel/manage_teams/')
...
def manage_teams(request):
    print "Entered in to manage teams functionality"
    user = request.user
    team_name = ''
    print request.POST
    ...
    if data.has_key('team_name') and data.has_key('edit_team_name') and data['team_name'] and data['edit_team_name']:
        ...
        try:
            ...
            for member in group_members:
                ...
                member.save()
            logger.info("%s changed team name from %s to %s !" %(user.email, data['team_name'], data['edit_team_name']))
            log_visit(request=request, action = 'EDIT_TEAM_NAME', email= user.email, source="WEB")
        except Exception, e:
            print str(e)
            pass
    ...
    try:
        ...
        print "Groups are there and listed", result.count()
    except:
        result = []
    ...
    try:
        ...
        if data.has_key('email') and data.has_key('username') and data['email'] and data['username']:
            try:
                ...
                g.save()
                logger.info("%s added  %s to %s team !" %(user.email, data['email'], data['team_name']))
                log_visit(request=request, action = 'ADD_TEAM_MEMBER', email= user.email, source="WEB")
    ...
    print "teams", result.count()
    print "teamobjects", team_objects
    print "user", request.user
    response = render_to_response('teams.html', {'teams':result, 'team_objects': team_objects, 'selected_team':team_name, 'user':request.user})
    return response
...
def update_teams(request):
    user = request.user
    from embitel_framework.get_team_members import get_team_members
    from embitel_framework.team import teams
    team_name = ''

    result = get_team_members()
    result = teams()

    log_visit(request=request, action = 'UPDATE_TEAMS', email= user.email, source="WEB")
    return HttpResponseRedirect('/embitel/manage_teams/')
...
def delete_user(request, encoded_key1=None):
    ...
    try:
        ...
        logger.info("%s deleted  %s from %s team !" %(request.user.email, g.email, g.group_name))
        log_visit(request=request, action = 'DELETE_TEAM_MEMBER', email= request.user.email, source="WEB")
        g.delete()
        result = Groups.objects.all().order_by('group_name').values_list('group_name', flat='true').distinct()
        team_objects = Groups.objects.filter(group_name=team_name).order_by('username')
        ...
        return HttpResponseRedirect('/embitel/manage_teams/')
    print "deleted the user and redirecting to manage teams"
    return HttpResponseRedirect('/embitel/manage_teams/')
...
def disable_on_alternate_os(email=None, device_type=None):
    ...
    try:
        if device_type == 'android':
            ...
            log_visit(request=None, action = 'DELETE_ON_IOS', email = email)
        elif device_type == 'ios':
            GCMDevices.objects.get(email=email).delete()
            log_visit(request=None, action = 'DELETE_ON_ANDROID', email = email)
    except Exception,e:
...
def audio_streaming(request):
    ...
    try:
        ...
        print "written data"
        destination.close()
        commands.getoutput('chmod 777 %s' %file_path)
    ...
    log_visit(request=request, action = 'AUDIO_MESSAGE', email= from_email)
    return HttpResponse(json.dumps(result), content_type='application/json')
...
def register_urbanairship_devices(request, *args, **kwargs):
    ...
    log_visit(request=request, action = 'REGISTRATION_OR_UPDATE', email= email)
    return HttpResponse(json.dumps(result), content_type='application/json')
    ...
    try:
        ...
        try:
            ...
            if email:
                ...
                print "sent mail"
                log_visit(request=None, action = 'OTP', email=email)
            #try:
            #    import requests
...
def hrms_management(request):
    ...
    log_visit(request=request, action = 'HRMS_ACCESS', email= email)
    print result
    return HttpResponse(json.dumps(result), content_type='application/json')
...
def mailers(request, *args, **kwargs):
    ...
    log_visit(request=request, action = 'MAILERS', email= email)
    result = {'success': True}
    return HttpResponse(json.dumps(result), content_type='application/json')
...
def register_urbanairship_devices_get(request, *args, **kwargs):
    ...
    try:
        ...
        if email and otp:
            ...
            result = {'success': True }
            print "returning http response", result
            print json.dumps(result)
            return HttpResponse(json.dumps(result), content_type='application/json')

        print "HTTP RESPoNSE not returned Yet and processing"
        if device_type == 'android':
            try:
                gcm_device_id = data['gcm_device_id']
            except:
...
def get_gropus_and_members(request, *args, **kwargs):
    ...
    try:
        ...
        if data.has_key('group_name'):
            ...
            #log_visit(request=request, action = 'GROUP_MEMBERS', email= email)
        else:
            result = groups
            log_visit(request=request, action = 'GROUPS', email= email)
        '''
        for group in groups:
...
def verify_registered_users(request, *args, **kwargs):
    ...
    try:
        ...
        print str(e)
        result = {'success': False}
    log_visit(request=request, action = 'FRIENDS', email= email)
    return HttpResponse(json.dumps(result), content_type='application/json')
...
def verify_device_active(request, *args, **kwargs):
    ...
    try:
        model = get_model(email=email)
        device = model.objects.get(email=email)
        device.last_heartbeat = datetime.datetime.now()
        device.save()
...
#This webservice is to handle apns payload 
@csrf_exempt
def iOS_webservice(request, *args, **kwargs):
    ...
    try:
        result = {}
        message_obj = Messages.objects.get(message_id=message_id, gcm_devices__id=None)
        result['from_email'] = str(message_obj.from_email)
        result['email'] = str(message_obj.apns_devices.email)
        result['message'] = str(message_obj.message)
        result['message_type'] = str(message_obj.message_type)
        result['message_id'] = str(message_obj.message_id)
        if message_obj.message_type == 'notification':
            result['start_time'] = ''
            result['end_time'] = ''
            if message_obj.start_time:
                result['start_time'] = str(message_obj.start_time)
 
            if message_obj.start_time:
                result['end_time'] = str(message_obj.end_time)
            result['body'] = str(message_obj.body)
            result['location'] = str(message_obj.location)
...
#This webservice is deliver pendig messages when app comes foreground
@csrf_exempt
def iOS_pending_messages(request, *args, **kwargs):
    ...
    try:
        model = get_model(email=email)
        device = model.objects.get(email=email)
        message_type = ['message', 'buzz']
        pending_messages = Messages.objects.filter(apns_devices=device, message_type__in=message_type, delivered=False).order_by('id')
        result = list(pending_messages.values('from_email', 'message_id', 'message_type', 'message', 'apns_devices__email'))
...
def send_PN_to_device(request, *args, **kwargs):
    ...
    if message_type == 'notification':
        ...
        if data.has_key('end_time') and data['end_time']:
            ...
            end_time = datetime.datetime.today().replace(hour=23, minute=59, second=0, microsecond=0)
        #subject = data["subject"]
    ...
    #//Delivered status: notification sent from dashboard will not have from_email so the following logic will take care for marking delivered status
    if not devices and message_type == 'notification': #TODO: put a check if message_type is notification once mayank changes message_type
        devices = list(APNSDevices.objects.filter(email__in=[from_email], pn_status=True))
        devices = devices + list(GCMDevices.objects.filter(email__in=[from_email], pn_status=True))
    ...
    for device in devices:
        ...
        if data.has_key('delivered') and device.device_type=='ios':
            ...
            if message_type == 'message':
                ...
                COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert": %s },"device_tokens": %s, "delivered":"True", "message_id":"%s", "message_type":"%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, repr(message), [str(device.registration_id)], message_id, message_type)
            try:
                if from_device.device_type == 'ios':
            ...
            try:
                ...
                if len(message) > 30:
                    message = message[:30]+'...'
                COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":%s}, "device_tokens": %s, "message_id": "%s", "message_type":"%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, repr(message), [str(device.registration_id)], message_id, message_type)
                #if message_type == 'notification': #Removed this check coz we are using webservice
            ...
            try:
                ...
                try:
                    ...
                    if message_type == 'notification':
                        print "Here it isssssssssss", start_time
        ...
        if pending_messages:
            ...
            if device.device_type == 'ios':
                ...
                if len(pending_message.message) > 50:
                    message = str(pending_message.message)[:30]+'...'
                
                COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":%s}, "device_tokens": %s, "message_id": "%s", "message_type":"%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, repr(message), [str(device.registration_id)], str(pending_message.message_id), str(pending_message.message_type))
                if pending_message.message_type == 'invitation':
                    try:
                        username = str(Groups.objects.get(email=str(pending_message.from_email)).username)
                    except:
                        username = str(pending_message.from_email)
                    message = "Do you want to accept request from %s?" %username
                    COMMAND = """curl -X POST -u "%s:%s"   -H "Content-Type: application/json"   --data '''{"aps":{"alert":%s}, "device_tokens": %s,"email":"%s", "from_email":"%s", "message_id": "%s", "message_type":"%s"}'''    https://go.urbanairship.com/api/push/""" %(device.app_key, device.master_key, repr(message), [str(device.registration_id)], str(device.email), str(pending_message.from_email), str(pending_message.message_id), str(pending_message.message_type))
    ...
    result={'result':'successfully sent Push notification!'}
    if data.has_key('delivered'):
        if data.has_key('message_type') and data['message_type'] == 'notification':
            log_visit(request=request, action = 'ACK_NOTIFICATION', email= from_email)
        else:
            log_visit(request=request, action = 'ACK_MESSAGE', email= from_email)

    else:
        if data.has_key('message_type') and data['message_type'] == 'notification':
            log_visit(request=request, action = 'SENT_NOTIFICATION', email= from_email)
        else:
            log_visit(request=request, action = 'SENT_MESSAGE', email= from_email)
...
def set_PN_status(request):
    ...
    log_visit(request=request, action = 'PN_STATUS_CHANGE', email= email)
    return HttpResponse(json.dumps(result), content_type='application/json')
...
@login_required
@csrf_exempt
def kewl_dashboard_PN(request, *args, **kwargs):
    teams = Groups.objects.exclude(group_name='Others').values_list('group_name', flat='true').order_by('group_name').distinct()
    response = render_to_response('kewl_dashboard_PN.html', {'teams': teams, 'form': ConfigureNotificationsForm,  'user':request.user})
    return response

@login_required
@csrf_exempt
def kewl_dashboard_send_PN(request, *args, **kwargs):
    print request.POST
    data = request.POST
    teams = Groups.objects.exclude(group_name='Others').values_list('group_name', flat='true').order_by('group_name').distinct()
    form = ConfigureNotificationsForm(request.POST)
    if form.is_valid(): # All validation rules pass
        if data.has_key('start_time') and data['start_time']:
            try:
                start_time = datetime.datetime.strptime(data['start_time'].strip(), '%Y-%m-%d %H:%M')
                start_time = datetime.datetime.strftime(start_time, '%d-%m-%Y %I:%M %p')
                print type(start_time)
            except:
                start_time = datetime.datetime.strptime(data['start_time'].strip(), '%Y-%m-%d')
                start_time = datetime.datetime.strftime(start_time, '%d-%m-%Y %I:%M %p')
        else:
            start_time = ''
        ...
        if data.has_key('end_time') and data['end_time']:
            try:
                end_time = datetime.datetime.strptime(data['end_time'].strip(), '%Y-%m-%d %H:%M')
                end_time = datetime.datetime.strftime(end_time, '%d-%m-%Y %I:%M %p')
            except:
                end_time = datetime.datetime.strptime(data['end_time'].strip(), '%Y-%m-%d')
                end_time = datetime.datetime.strftime(end_time, '%d-%m-%Y %I:%M %p')
        else:
            end_time = ''

        if data.has_key('send_bulk_PN') and data['send_bulk_PN'] == 'Send to ALL':
            logger.info("%s sent Notifications to ALL users !" %request.user.email)
            log_visit(request=request, action = 'SEND_PN_TO_ALL', email= request.user.email, source="WEB")
            message_id = generate_rand()
            print "Sending to All"
            emails = Groups.objects.filter(installed=True).values_list('email', flat='true')
            count = len(emails)
            for email in emails:
                #TODO:Remove this check!!!!!!!!!!!!!!
                #if email == 'prakash.p@embitel.com':
                #different message ids??
                send_PN_to_device(request=None, email=email, message_type='notification', message_id=message_id, message=str(data.get('subject')), body=str(data.get('message')), start_time=start_time, end_time=end_time, location=str(data.get('location')), from_email='', app_key='a', secret_key='b')
             

        elif data.has_key('send_bulk_PN') and data['send_bulk_PN'] == 'Send':
            logger.info("%s sent Notifications to SELECTED teams !" %request.user.email)
            log_visit(request=request, action = 'SEND_PN_TO_TEAMS', email= request.user.email, source="WEB")
            message_id = generate_rand()
            group_names = data.getlist('group_names')
            emails = Groups.objects.filter(group_name__in = group_names, installed=True).values_list('email', flat='true')
            count = len(emails)
            for email in emails:
                #if email == 'prakash.p@embitel.com': #TODO: remove this check
                send_PN_to_device(request=None, email=email, message_type='notification', message_id=message_id, message=str(data.get('subject')), body=str(data.get('message')), start_time=start_time, end_time=end_time, location=str(data.get('location')), from_email='', app_key='a', secret_key='b')
            print "Sending to selected teams"
        else:
            count = 0
            print "Sening to slected users"
        
        response = render_to_response('kewl_notifications_status.html', {'status': "Successfully sent notifications to %s users!" %count})

        log_visit(request=request, action = 'SENT_NOTIFICATION', email= request.user.email, source="WEB")
        return response
        #return HttpResponseRedirect('/embitel/dashboard/configure_notifications/')
    else:
        response = render_to_response('kewl_dashboard_PN.html', {'teams': teams, 'form': form, 'user':request.user})
        return response
